    ...
    public class SyncInProgressOrderFromViewJob : IJob
    {
        ...
        public List<OrderItemResponse> GetWebsaleOrderFromView()
        {
            ...
            string query = $@"SELECT ORDER_ID, DELIVERY_CODE, TIMEIN, TIMEOUT, LOADWEIGHTNULL, STATUS, LOADWEIGHTFULL, PRODUCT_NAME, 
                                     ...
                                     TRANSPORT_METHOD_ID, LAST_UPDATE_DATE, ITEM_CATEGORY, DOC_NUM, ORDER_REQ_ID, BLANKET_ID, SIDE_SEAL_COUNT, SIDE_SEAL_DES
            ...
            OrderItemResponse mapFunc(IDataReader reader) => new OrderItemResponse
            {
                ...
                sideSealCount = int.TryParse(reader["SIDE_SEAL_COUNT"]?.ToString(), out int ssc) ? ssc : default,
                sideSealDes = reader["SIDE_SEAL_DES"].ToString()
            };
        ...
        public async Task<bool> SyncWebsaleOrderToDMS(OrderItemResponse websaleOrder)
        {
            ...
            else if (stateId == (int)OrderState.DA_XUAT_HANG)
            {
                ...
                {
                    isSynced = await _storeOrderOperatingRepository.UpdateReceivedOrder(websaleOrder.id, websaleOrder.timeOut, websaleOrder.loadweightfull, websaleOrder.sideSealCount, websaleOrder.sideSealDes, websaleOrder.docnum);
