...
using XHTD_SERVICES_XB_TROUGH_6.Models.Response;
...
namespace XHTD_SERVICES_XB_TROUGH_6.Jobs
{
    public class TroughJob : IJob
    {
        ...
        private readonly string TROUGH_CODE = "6";
                    ...
                    if (openResult != 0)
                    {
                        ...
                        if (Program.CountToSendFailOpenPort == 3)
                        {
                            ...
                            if (Program.SendFailOpenPortLastTime == null || Program.SendFailOpenPortLastTime < DateTime.Now.AddMinutes(-3))
                            {
                                ...
                                var pushMessage = $"Xi bao 6: mở kết nối không thành công đến anten {PegasusAdr}. Vui lòng báo kỹ thuật kiểm tra";
