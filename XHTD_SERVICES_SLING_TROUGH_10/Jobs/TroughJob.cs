...
using XHTD_SERVICES_SLING_TROUGH_10.Models.Response;
...
using Autofac;
using XHTD_SERVICES_SLING_TROUGH_10.Hubs;
                    ...
                    if (openResult != 0)
                    {
                        ...
                        if (Program.CountToSendFailOpenPort == 3)
                        {
                            ...
                            if (Program.SendFailOpenPortLastTime == null || Program.SendFailOpenPortLastTime < DateTime.Now.AddMinutes(-3))
                            {
                                ...
                                var pushMessage = $"Sling 10: mở kết nối không thành công đến anten {PegasusAdr}. Vui lòng báo kỹ thuật kiểm tra";
