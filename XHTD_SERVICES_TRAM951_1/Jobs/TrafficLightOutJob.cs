...
{
    public class TrafficLightOutJob : IJob
    {
        ...
        protected readonly string DEVICE_CODE = "951-1.DGT-OUT";

        public TrafficLightOutJob(CategoriesDevicesRepository categoriesDevicesRepository,
        ...
        public async Task Execute(IJobExecutionContext context)
        {
            ...
            {
                _logger.LogInfo("Start tramcan1 traffic light OUT 951 - 1 service");
                _logger.LogInfo("----------------------------");
        ...
        public void TrafficLightProcess()
        {
            try
            {
                ...
                {
                    if (Program.IsFirstTimeResetTrafficLightOut)
                    {
                        Program.IsFirstTimeResetTrafficLightOut = false;

                        TurnOnTrafficLight();

                        _logger.LogInfo("Reset traffic light OUT - Scale 951 - 1");
                    }
                    else
                    {
                ...
                {
                    Program.IsFirstTimeResetTrafficLightOut = true;
                }
            }
            catch (Exception ex)
            {
