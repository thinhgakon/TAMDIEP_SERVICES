    ...
    public class SyncTroughJob12 : IJob, IDisposable
    {
        ...
        public async Task SyncTroughProcess()
        {
            try
            {
                ...
                {
                    WriteLogInfo($"Trough Job MDB 1|2: Ket noi thanh cong --- IP: {IP_ADDRESS} --- PORT: {PORT_NUMBER}");
                ...
                {
                    WriteLogInfo($"Trough Job MDB 1|2: Ket noi that bai --- IP: {IP_ADDRESS} --- PORT: {PORT_NUMBER}");
                }
            }
            catch (Exception ex)
            {
                WriteLogInfo($"Trough Job MDB 1|2: ERROR --- IP: {IP_ADDRESS} --- PORT: {PORT_NUMBER}: {ex.Message} -- {ex.StackTrace}");
            }
            finally 
            {
                if (client != null)
        ...
        public async Task ReadDataFromMachine(List<string> machineCodes)
        {
            foreach (var machineCode in machineCodes)
            {
                try
                {
                    ...
                    SendPLCNotification("PLC", machineCode, plcStatus, null, deliveryCode, (int?)firstSensorQuantity);
        ...
        public void SendPLCNotification(string machineType, string machineCode, string startStatus, string stopStatus, string deliveryCode, int? quantity)
        {
            _notification.SendMachineNotification(machineType, machineCode, startStatus, stopStatus, deliveryCode, quantity);
        }
