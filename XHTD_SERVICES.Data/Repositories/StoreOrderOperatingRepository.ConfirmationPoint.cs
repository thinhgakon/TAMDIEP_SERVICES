        ...
        public void UpdateIndexOrderForNewConfirm(string vehicleCode)
        {
            try
            {
                ...
                using (var db = new XHTD_Entities())
                {
                    ...
                    var orderIndexMax = db.tblStoreOrderOperatings.Where(x => (x.Step == (int)OrderStep.DA_XAC_THUC || x.Step == (int)OrderStep.DANG_GOI_XE) && (x.IndexOrder2 ?? 0) == 0 && x.TypeProduct.Equals(currentOrder.TypeProduct)).Max(x => x.IndexOrder) ?? 0;
                    // log thêm các đơn cùng loại đã được xếp lốt
                    var orderReceivings = db.tblStoreOrderOperatings.Where(x => (x.Step == (int)OrderStep.DA_XAC_THUC || x.Step == (int)OrderStep.DANG_GOI_XE) && (x.IndexOrder2 ?? 0) == 0 && x.TypeProduct.Equals(currentOrder.TypeProduct)).ToList();
