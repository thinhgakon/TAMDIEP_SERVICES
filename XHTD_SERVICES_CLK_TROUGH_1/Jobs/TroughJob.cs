...
using XHTD_SERVICES_CLK_TROUGH_1.Models.Response;
...
using Autofac;
using XHTD_SERVICES_CLK_TROUGH_1.Hubs;
...
namespace XHTD_SERVICES_CLK_TROUGH_1.Jobs
{
    public class TroughJob : IJob
    {
        ...
        private readonly string CAMERA_IP = "192.168.13.238";
        ...
        private readonly string MACHINE_CODE = "9";
                    ...
                    if (openResult != 0)
                    {
                        ...
                        if (Program.CountToSendFailOpenPort == 3)
                        {
                            ...
                            if (Program.SendFailOpenPortLastTime == null || Program.SendFailOpenPortLastTime < DateTime.Now.AddMinutes(-3))
                            {
                                ...
                                var pushMessage = $"Clinker: mở kết nối không thành công đến anten {PegasusAdr}. Vui lòng báo kỹ thuật kiểm tra";

                                _logger.LogInfo($"Gửi cảnh báo: {pushMessage}");

                                SendNotificationByRight(RightCode.TROUGH_CLINKER, pushMessage, "SYSTEM");
                            }
