...
namespace XHTD_SERVICES_QUEUE_TO_GATEWAY.Jobs
{
    public class QueueToGatewayJob : IJob
    {
        ...
        public QueueToGatewayJob(StoreOrderOperatingRepository storeOrderOperatingRepository)
        {
        ...
        public void QueueToCallProccess(string typeProduct)
        {
            try
            {
                using (var db = new XHTD_Entities())
                {
                    ...
                    foreach (var callConfig in callConfigs)
                    {
                        int sourceDocumentId = (int)callConfig.SourceDocumentId;
                        var typeProductConfig = db.tblTypeProductCallToGatewayConfigs
                                                  .FirstOrDefault(x => x.CallToGatewayConfigId == callConfig.Id &&
                                                                       x.TypeProductCode.ToUpper() == typeProduct.ToUpper());

                        var maxVehicleConfig = typeProductConfig != null ? (int)typeProductConfig.MaxVehicleNumber : 5;

                        ProcessByExportPlan(maxVehicleConfig, sourceDocumentId, typeProduct);
                    }
                }
            }
            catch (Exception ex)
        ...
        public void ProcessByExportPlan(int LimitVehicle, int sourceDocumentId, string typeProduct)
        {
            ...
            try
            {
                ...
                var orders = _storeOrderOperatingRepository.CountStoreOrderWaitingIntoTroughByTypeAndExportPlan(typeProduct, sourceDocumentId);
                WriteLogInfo($@"1. Các đơn {typeProduct} đang trong nhà máy hiện tại: {string.Join(",", orders.Select(x => x.DeliveryCode))}");
                ...
                AddVehicleIntoQueue(LimitVehicle - vehicleFrontYard, sourceDocumentId, typeProduct);
            }
            catch (Exception ex)
            {
                WriteLogInfo($@"ProcessByExportPlan {typeProduct}: {ex.Message}");
            }
        }

        public void AddVehicleIntoQueue(int topX, int? sourceDocumentId, string typeProduct)
        {
            try
            {
                ...
                using (var db = new XHTD_Entities())
                {
                    ...
                    var query = db.tblStoreOrderOperatings
                                    .Where(x => x.Step == (int)OrderStep.DA_XAC_THUC &&
                                                x.TypeProduct.Equals(typeProduct) &&
                                                x.IsVoiced == false);
                    ...
                    {
                        WriteLogInfo($"3. Không tìm thấy xe {typeProduct} nào với số hiệu hợp đồng: {sourceDocumentId} => Kết thúc");
                        return;
                    }
                    else
                    {
