...
{
    public class QueueToGatewayExportPlanJumboJob
    {
        private static readonly log4net.ILog log = log4net.LogManager.GetLogger("QueueToCallPcb30FileAppender");
        private const string TYPE_PRODUCT = "C91";
        ...
        public async Task Execute(IJobExecutionContext context)
        {
            ...
            await Task.Run(() =>
            {
                log.Info($@"Start QueueToCallPcb30Job: {DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss")}");
                QueueToCallProccess();
            });
        }
        ...
        public void QueueToCallProccess()
        {
            try
            {
                ...
                using (var db = new XHTD_Entities())
                {
                    ...
                    {
                        int exportPlanId = (int)itemConfig.ExportPlanId;
                        int maxVehicleJumbo = (int)itemConfig.MaxVehicleJumbo;
                        ProcessByExportPlan(maxVehicleJumbo, exportPlanId);
                    }
                }
            }
            catch (Exception ex)
        ...
        public void ProcessByExportPlan(int LimitVehicle, int exportPlanId)
        {
            log.Info($@"Xử lý kế hoạch {exportPlanId} với cấu hình xe tối đa {LimitVehicle}");
            try
            {
                //get sl xe trong bãi chờ máng ứng với sp
                var vehicleFrontClinkerYard = 0; //_storeOrderOperatingRepository.CountStoreOrderWaitingIntoTroughByTypeAndExportPlan(TYPE_PRODUCT_NAME, exportPlanId);

                log.Info($@"1. Số xe trong bãi chờ máng: {vehicleFrontClinkerYard}");
                ...
                AddVehicleIntoQueue(LimitVehicle - vehicleFrontClinkerYard, exportPlanId);
            }
            catch (Exception ex)
            {
        ...
        public void AddVehicleIntoQueue(int topX, int exportPlanId)
        {
            try
            {
                using (var db = new XHTD_Entities())
